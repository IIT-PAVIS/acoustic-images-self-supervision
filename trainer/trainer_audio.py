...
class Trainer(object):
    ...
    def __init__(self, model_1, model_2, model_transfer, logger=None, display_freq=1,
    ...
    def _build_functions(self, data):
        ...
        logits_2_multiple = tf.tile(self.model_2.output, [1, 12 * 16])
        ...
        logits_transfer_reshape = tf.reduce_mean(tf.reshape(logits_transfer, shape=expanded_shape), axis=1)
        transferweighted = tf.reduce_sum(logits_transfer_reshape, axis=[1, 2])
        self.transfer = tf.nn.l2_normalize(transferweighted, dim=1)
        ...
        self.audio = tf.nn.l2_normalize(self.model_2.output, dim=1)
        self.tripletlossacoustic, _ = tf.cond(self.epoch > 3,
                                              lambda: self.mix_data_hard(self.audio, self.transfer, self.labels,
    ...
    def train(self, train_data=None, valid_data=None):
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True,
            ...
            for epoch in range(start_epoch, start_epoch + self.num_epochs):
                ...
                while True:
                    try:
                        tripletlossvalue, tripletlossacoustic, train_loss, train_summary, _, _ = session.run(  # train_accuracy,
                            [self.tripletloss, self.tripletlossacoustic, self.loss, self.logger.summary_op, self.train_op_0, self.train_op_1],  #
                        ...
                        if step % self.display_freq == 0:
                            print('{}: {} - Iteration: [{:3}]\t Training_Loss: {:6f}\t Triplet_Loss: {:6f}\t Triplet_acoustic_Loss: {:6f}'.format(
                                # \t Training_Accuracy: {:6f}
                                datetime.now(), FLAGS.exp_name, step, train_loss, tripletlossvalue, tripletlossacoustic))  # , train_accuracy
    ...
    def mix_data_hard(self, data0, data1, labels, scenario, margin, distance):  # acoustic_data and video_data
        # compute distances
        if distance:
    ...
    def mix_all(self, data0, data1, labels, scenario, margin, distance):
    ...
    def _retrieve_batch(self, next_batch):
        ...
        return acoustic_data, video_data, acoustic_tr_data, labels, scenario
