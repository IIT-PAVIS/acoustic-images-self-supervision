...
flags.DEFINE_integer('total_length', 30, 'Length in seconds of total sequence sample')
flags.DEFINE_integer('number_of_crops', 15, 'Length in seconds of a sequence sample')
flags.DEFINE_integer('num_class', 128, 'Classes')
flags.DEFINE_string('mode', 'testing', 'training or testing or validation')
flags.DEFINE_string('epoch', '0', 'Epoch')
FLAGS = flags.FLAGS


def main(_):
    epoch = FLAGS.epoch
    # if FLAGS.mode == 'testing' and (epoch == 'random' or epoch == '0' or epoch == '10' or epoch == '100'):
    #     savemap = True
    # else:
    savemap = False
    features(FLAGS.mode, savemap)
...
def features(dataset, savemap):
    ...
    name2 = '{}_{}'.format(FLAGS.model2, dataset)
    data_dir2 = str.join('/', FLAGS.init_checkpoint.split('/')[:-1] + [name2]) + '_' + name
    ...
    if os.path.exists(data_dir2):
        ...
        os.makedirs(data_dir2)  # mkdir creates one directory, makedirs all intermediate directories
    ...
    name_file = '/data/vsanguineti/dualcam_actions_dataset2/30_seconds/lists/{}.txt'.format(dataset)
    ...
    scenario = tf.reshape(next_batch[4],
                          shape=[-1, 3])
    person = tf.reshape(next_batch[5],
                        shape=[-1, 9])
    ...
    with tf.Session(
        ...
        dataset_audio_list_features = np.zeros([data_size, num_embedding], dtype=float)
        dataset_video_list_features = np.zeros([data_size, num_embedding], dtype=float)
        dataset_labels = np.zeros([data_size, numcl], dtype=int)
        dataset_scenario = np.zeros([data_size, num_scenario], dtype=int)
        dataset_people = np.zeros([data_size, people], dtype=int)
        if savemap:
            dataset_map = np.zeros([data_size, size1, size2, num_embedding], dtype=float)
            dataset_images = np.zeros([data_size, size11, size22, size33], dtype=float)
        ...
        while True:
            try:
                ...
                labels_data, scenario_data, person_data, features_audio, features_video, images, product = session.run(
                    [label, scenario, person, productvectnorm2, productvectnorm,
                     data2, innerdot],
                ...
                dataset_people[total_size:total_size + batchnum, :] = person_data
                if savemap:
                    # take one image for each block
                    dataset_images[total_size:total_size + batchnum, :, :, :] = images[::FLAGS.nr_frames]
    ...
    np.save('{}/{}_{}_data.npy'.format(data_dir1, FLAGS.model1, dataset), dataset_audio_list_features)
    np.save('{}/{}_{}_data.npy'.format(data_dir2, FLAGS.model2, dataset), dataset_video_list_features)
    np.save('{}/{}_{}_labels.npy'.format(data_dir2, FLAGS.model2, dataset), dataset_labels)
    np.save('{}/{}_{}_scenario.npy'.format(data_dir2, FLAGS.model2, dataset), dataset_scenario)
    np.save('{}/{}_{}_person.npy'.format(data_dir2, FLAGS.model2, dataset), dataset_people)
    np.save('{}/{}_{}_labels.npy'.format(data_dir1, FLAGS.model1, dataset), dataset_labels)
    np.save('{}/{}_{}_scenario.npy'.format(data_dir1, FLAGS.model1, dataset), dataset_scenario)
    np.save('{}/{}_{}_person.npy'.format(data_dir1, FLAGS.model1, dataset), dataset_people)
    if savemap:
        np.save('{}/{}_{}_images.npy'.format(data_dir2, FLAGS.model2, dataset), dataset_images)
        np.save('{}/{}_{}_maps.npy'.format(data_dir2, FLAGS.model2, dataset), dataset_map)
